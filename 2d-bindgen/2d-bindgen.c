...
void process_namespace(GIRepository* repo, const gchar* namespace)
{
	...
	{
		fmt_line("pub type gboolean = u32;");
		fmt_line("pub type interface = ();");
	}
	...
	fmt_line("#[path=\"gen-%s.rs\"]", namespace);
	fmt_line("mod raw;");
	fmt_line("");
...
int main(int argc, char** argv)
{
	...
	if(argc>1)
	{
		...
		if(mkdir(libname, S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH))
		{
			if(errno != EEXIST)
			{
				fprintf(stderr, "could not create directory %s: %s", libname, strerror(errno));
				return;
			}
		}
		char* filename=g_strdup_printf("%s/lib.rs", libname);
		out = fopen(filename, "w");
		free (filename);
		...
		fmt_line("#[desc = \"Rust bindings for %s, generated by 2d-bindgen\"];", libname);
